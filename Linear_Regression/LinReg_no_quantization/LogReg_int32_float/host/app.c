...
static void compute_mae(const T* X, const T* Y, const float* W, int m_size, int n_size, 
    const char* comment) { 
    float reduction = 0; 
    float sum_of_Y = 0; 
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        ...
        for (uint32_t j = 0; j < m_size; ++j) {
            ...
            for (unsigned int l = 0; l < n_size; ++l) {
                ...
                // if (j == 0) 
                //     printf("inidival gradient: %f\n", X[j*n_size + l] * (Y[j] - dot_product) / m_size); 
            }
        } // gradient done 
        ...
        // printf("i: %d, g: %.4f, g*lr: %.4f, w: %.4f\n", i, gradient_tmp[0], ((float) gradient_tmp[0] * lr), W[0]); 
...
int main(int argc, char **argv) {
    ...
    compute_mae(bufferX, bufferY, W_dpu_float, m_size, n_size, "DPUs"); 
    # endif
