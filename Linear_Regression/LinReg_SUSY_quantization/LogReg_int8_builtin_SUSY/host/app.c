...
* With modified multiplication of int16 (8-bit based), builtin used 
*/
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
static int read_input_SUSY(T* X, T* Y, unsigned int m_size, unsigned int n_size) {
    ...
    fp = fopen("/home/upmem0013/gyuxin/SUSY_int8.csv", "r"); // add file path 
    if (fp == NULL) {
...
static void GD_host_fp(T* X, T* Y, int16_t* W, float* W_float, uint32_t m_size, uint32_t n_size, 
    uint32_t iter_time, float lr) 
{
    printf("-----Start traing of SUSY at host, int8-----\n"); 
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        // calculate gradient 
        int32_t* gradient_tmp = (int32_t*) calloc(n_size, sizeof(int32_t)); 
        ...
        // printf("%d: gradient0 = %d, W0 = %d, W_float0 = %f\n", i, gradient_tmp[0], W[0], W_float[0]); 
...
int main(int argc, char **argv) {
    ...
    W = malloc(n_size_pad * sizeof(int16_t)); 

    // init trainging dataset and weight for host 
    T *bufferX = X;
    T *bufferY = Y;
    int16_t *bufferW_host = W; 
