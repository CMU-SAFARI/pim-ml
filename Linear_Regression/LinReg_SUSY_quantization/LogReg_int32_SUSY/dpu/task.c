/*
 * Compute gradient of MSE loss function with multiple tasklet, quantized SUSY dataset 
 * int32, code generated by compiler 
 */
#include <stdint.h>
#include <stdio.h>
#include <defs.h>
...
__mram_noinit int64_t DPU_RESULTS[MAX_ROWS]; // partial gradient in each DPU, max number of rows = 16 

__dma_aligned int64_t gradient_tmp[MAX_ROWS*NR_TASKLETS]; // tasklet major storage

// Dot product
static int64_t dot_product(T *bufferX, int32_t *bufferW, uint32_t length) { 
    int64_t result = 0; 
...
int main() {
    ...
    T *cache_W = (T *) mem_alloc(n_size_pad << MUL); // fixed-point arithmetic 

    // read W from MRAM 
    mram_read((__mram_ptr void const*) (mram_base_addr_W), cache_W, n_size_pad << MUL); 
    ...
    for (unsigned int row_index = 0; row_index < rows_per_tasklet;) {
        ...
        for(unsigned int y_index = 0; (y_index<rows_per_cache) && (row_index<rows_per_tasklet); y_index++, row_index++){
            ...
            // compute dot product
            int64_t dot_product_t = dot_product(cache_X + x_index, cache_W, n_size); 
            // compute gradient 
            for (unsigned int l = 0; l < n_size; ++l) { 
                gradient_tmp[tasklet_offset + l] -= cache_X[x_index + l] * ((cache_Y[y_index] 
                    << SHIFT_AMOUNT) - dot_product_t) >> OVERFLOW_SHIFT; 
		
            } 
            # if PRINT
            if(row_index < 4) {
                printf("dot_product dpu: %lld\n", dot_product_t); 
                printf("X at DPU: "); 
                for (uint32_t each_attribute = 0; each_attribute < n_size_pad; each_attribute++) {
    ...
    if (tasklet_id == 0) {
        ...
        mram_write((const void *) gradient_tmp, (__mram_ptr void *) DPU_RESULTS, (n_size_pad << 3)); 
    } 
