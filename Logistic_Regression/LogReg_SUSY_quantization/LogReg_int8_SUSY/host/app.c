...
// static void read_input(T* X, T* Y, int16_t* W, unsigned int m_size, unsigned int n_size) {
//     srand(0);
...
static int read_input_SUSY(T* X, T* Y, unsigned int m_size, unsigned int n_size) {
    ...
    fp = fopen("/home/upmem0013/gyuxin/SUSY_log_int8.csv", "r"); // add file path of SUSY.csv 
    if (fp == NULL) {
    ...
    fclose(fp); 
    printf("Successfully generate input data, # of rows = %d\n", m); 
    if (m != m_size) {
...
static void GD_host_fp(T* X, T* Y, int16_t* W, float* W_float, 
    uint32_t m_size, uint32_t n_size, uint32_t iter_time, float lr) 
{
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        ...
        for (uint32_t j = 0; j < m_size; ++j) {
            ...
            #if PRINT
            if (j < 2) {
                printf("dot_product = %d, sigmoid = %d\n", dot_product, sigmoid_temp); 
                printf("X at host: "); 
        ...
        printf("i: %d, g: %d, g*lr/m_size: %.4f, w: %d, w_float: %.4f\n", i, gradient_tmp[0], \
            (float) gradient_tmp[0] * lr / (m_size>>OVERFLOW_SHIFT), W[0], W_float[0]); 
