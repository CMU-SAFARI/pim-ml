/**
* app.c
* LogReg Host Application Source File
* int32, quantized SUSY  
* Symmetric 16-bit look-up-table for sigmoid in DPUs, load to MRAM and transfered to WRAM at the beginning 
* 
*/
...
// static void read_input(T* X, T* Y, int32_t* W, unsigned int m_size, unsigned int n_size) {
//     srand(0);
...
//             dot_product += X[j*n_size + k] * W[k] + (rand()%40 - 20); 
//         }
...
static int read_input_SUSY(T* X, T* Y, unsigned int m_size, unsigned int n_size) {
    ...
    fp = fopen("/home/upmem0013/gyuxin/SUSY_log_int16.csv", "r"); // add file path of SUSY.csv 
    if (fp == NULL) {
...
static void GD_host_fp(T* X, T* Y, int32_t* W, float* W_float, 
    uint32_t m_size, uint32_t n_size, uint32_t iter_time, float lr) 
{
    printf("-----Start traing at host, 8-bit (hybrid) fixed-point arithmetic-----\n"); 

    // init weight with random value
    for (uint32_t n = 0; n < n_size; ++n){
        W[n] = 1; //(1 << SHIFT_AMOUNT); 
    }
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        ...
        for (uint32_t j = 0; j < m_size; ++j) {
            ...
            int16_t sigmoid_temp = (int16_t) round((1<<SHIFT_AMOUNT)/(1.0 + exp((double) -dot_product/(1<<SHIFT_AMOUNT)))); 

            for (unsigned int l = 0; l < n_size; ++l) {
                // avoid overflow
                gradient_tmp[l] += X[j*n_size + l] * (sigmoid_temp - (Y[j]<<SHIFT_AMOUNT)) >> OVERFLOW_SHIFT; 
            } 
        } // gradient done 
        ...
        // printf("i: %d, g: %d, g*lr/m_size: %.4f, w: %d, w_float: %.4f\n", i, gradient_tmp[0], (float) gradient_tmp[0] * lr / (m_size>>OVERFLOW_SHIFT), W[0], W_float[0]); 
...
static void init_LUT(int16_t* sigmoid_lut, int32_t size_half, int32_t shift_value){
    for(int32_t i = 1; i <= size_half; ++i){
        sigmoid_lut[i-1] = (int16_t) round(shift_value / (1.0 + exp((double) -i/shift_value))); 
    }
} 
...
void compute_error_rate(const T* X, const T* Y, const float* W, int m_size, int n_size, char* comment){
    ...
    for (int m = 0; m < m_size; ++m) {
        ...
        double sigmoid_temp = 1 / (1 + exp((double)(-dot_product))); 
        T predict_temp = sigmoid_temp >= 0.5 ? 1 : 0; 
        if(predict_temp != Y[m]){
            reduction++; 
        }
        sum_of_Y += Y[m]; 
...
int main(int argc, char **argv) {
    ...
    W_fp = malloc(n_size_pad * sizeof(int32_t)); 
    ...
    // read_input(bufferX, bufferY, W_fp, m_size, n_size); 

    // init Weight for DPU 
    // T* W_dpu = malloc(n_size_pad * sizeof(T)); 
    int32_t* W_dpu_fp = (int32_t*) malloc(n_size_pad * sizeof(int32_t)); 
    ...
    // temp dpu gradient  
    int32_t* gradient_dpu_tmp = (int32_t*) malloc(n_size_pad * nr_of_dpus * sizeof(int32_t)); 

    // init LUT for sigmoid function  
    int16_t* sigmoid_lut = (int16_t*) malloc(LUT_SIZE * sizeof(int16_t)); 
    init_LUT(sigmoid_lut, SIGMOID_BOUNDARY<<SHIFT_AMOUNT, 1<<SHIFT_AMOUNT); 
    ...
    start(&timer, 0, 0); 
    GD_host_fp(bufferX, bufferY, W_fp, W_host_float, m_size, n_size, iter_time, learning_rate); 
    stop(&timer, 0); 
    ...
    // Transfer LUT 
    printf("Load sigmoid LUT to DPUs\n");
    DPU_FOREACH(dpu_set, dpu, i) {
        DPU_ASSERT(dpu_prepare_xfer(dpu, sigmoid_lut)); 
    }
    DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, "SIGMOID_LUT", 0, \
        (SIGMOID_BOUNDARY<<SHIFT_AMOUNT) * sizeof(int16_t), DPU_XFER_DEFAULT)); 
    ...
    for(uint32_t rep = 0; rep < iter_time; ++rep) {
        ...
        DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, \
            ...
            n_size_pad * sizeof(int32_t), DPU_XFER_DEFAULT)); 
        ...
        if (rep%1 == 0) {
            ...
            printf("iter %d, Display DPU Logs\n", rep); 
    ...
    for (uint32_t m = 0; m < n_size; ++m) {
        W_dpu_float[m] = (float) W_dpu_fp[m] / (1<<SHIFT_AMOUNT); 
...
// #endif   
    ...
    compute_error_rate(bufferX, bufferY, W_host_float, m_size, n_size, "host"); 
    compute_error_rate(bufferX, bufferY, W_dpu_float, m_size, n_size, "DPUs"); 
