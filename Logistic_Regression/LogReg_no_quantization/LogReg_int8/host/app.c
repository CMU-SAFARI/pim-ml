/**
* app.c
* LogReg_3.0 Host Application Source File
* int8 
...
static void GD_host_fp(T* X, T* Y, int16_t* W, uint32_t m_size, uint32_t n_size, uint32_t iter_time, float lr) {
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        ...
        for (uint32_t j = 0; j < m_size; ++j) {
            ...
            int16_t sigmoid_temp = (int32_t) round((1<<SHIFT_AMOUNT)/(1.0 + exp((double) -dot_product/(1<<SHIFT_AMOUNT)))); 
            ...
            #if PRINT
            if(j < 100){
                printf("%d, dot_product at host %d, s at host: %d\n", j, dot_product, sigmoid_temp); 
                printf("X at host: "); 
...
static void init_LUT(int16_t* sigmoid_lut, int32_t size_half, int32_t shift_value){
    for(int32_t i = 1; i <= size_half; ++i){
        ...
        // sigmoid(-i)
        // sigmoid_lut[i+size_half-1] = (int32_t) round(shift_value / (1.0 + exp((double) i/shift_value))); 
        // if(i < 100)
        //     printf("%d, %f, %d, %d\n", i, shift_value / (1.0 + exp((double) i/shift_value)), sigmoid_lut[i-1], \
        //         sigmoid_lut[i+size_half-1]); 
    }
} 
...
int main(int argc, char **argv) {
    ...
    W_fp = malloc(n_size_pad * sizeof(int16_t)); 
    ...
    read_input(bufferX, bufferY, bufferW_host, m_size, n_size); 
    // read_input_Skin(bufferX, bufferY, bufferW_host, m_size, n_size); 
    ...
    for (uint32_t n = 0; n < n_size_pad; ++n) {
        W_dpu[n] = (T) 1.0; 
        W_dpu_fp[n] = (int16_t) (1 << SHIFT_AMOUNT); 
    }
    ...
    start(&timer, 0, 0); 
    GD_host_fp(bufferX, bufferY, W_fp, m_size, n_size, iter_time, learning_rate); 
    stop(&timer, 0); 
    ...
    for (uint32_t x = 0; x < n_size; ++x) {
        W_host_float[x] = (float) W_fp[x] / (1<<SHIFT_AMOUNT); 
