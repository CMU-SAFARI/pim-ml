/**
* app.c
* LogReg_2.3 Host Application Source File
* int32 and float 
...
static void GD_host_fp(T* X, T* Y, T* W, uint32_t m_size, uint32_t n_size, uint32_t iter_time, float lr) {
    ...
    for (uint32_t i = 0; i < iter_time; ++i) {
        ...
        for (uint32_t j = 0; j < m_size; ++j) {
            ...
            T sigmoid_temp = (int32_t) round((1<<SHIFT_AMOUNT)/(1.0 + exp((double) -dot_product/(1<<SHIFT_AMOUNT)))); 
...
int main(int argc, char **argv) {
    ...
    for (uint32_t n = 0; n < n_size; ++n){
        // W_predef[n] = 0; 
        W_predef[n] = (float) bufferW_host[n]; 
    }
    ...
    #ifdef INT32 
    int16_t* sigmoid_lut = (int16_t*) malloc((SIGMOID_BOUNDARY<<SHIFT_AMOUNT) * sizeof(int16_t)); 
    ...
    //     #ifdef FLOAT
    //     printf("%.4f, ", (float) bufferW_host[x]); 
    //     #else
    //     W_host_float[x] = (float) (bufferW_host[x]>>SHIFT_AMOUNT) + \
    //                         ((float)(bufferW_host[x]&SHIFT_MASK)/(1<<SHIFT_AMOUNT));
    //     printf("%.4f, ", W_host_float[x]); 
    //     #endif
    // }
    // printf("\n"); 
