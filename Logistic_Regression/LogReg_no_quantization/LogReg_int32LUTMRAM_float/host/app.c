/**
* app.c
* LogReg_2.1 Host Application Source File
* int32 and float 
* Look-up-table for sigmoid in DPUs, accessed by mram_read
* 
*/
#include <stdio.h>
#include <stdlib.h>
...
// Assume the boundary is -+20, size of LUT = 40*2048*4 = 320KB 
static void init_LUT(T* sigmoid_lut, int32_t size_half, int32_t shift_value){
    for(int32_t i = 1; i <= size_half; ++i){
        // sigmoid(i) 
        sigmoid_lut[i-1] = (int32_t) round(shift_value / (1.0 + exp((double) -i/shift_value))); 
        // sigmoid(-i)
...
int main(int argc, char **argv) {
    ...
    #ifdef INT32 
    T* sigmoid_lut = (T*) malloc((SIGMOID_BOUNDARY<<(SHIFT_AMOUNT+1)) * sizeof(T)); 
    ...
    DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, "SIGMOID_LUT", 0, \
        (SIGMOID_BOUNDARY<<(SHIFT_AMOUNT+1)) * sizeof(T), DPU_XFER_DEFAULT)); 
    #endif
