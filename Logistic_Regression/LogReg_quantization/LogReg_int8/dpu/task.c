 ...
 * Code generated by compiler, no builtin 
 */
...
int main() {
    ...
    if(tasklet_id == NR_TASKLETS-1) {
        ...
        printf("n_size_pad_byte: %d, cache size: %d, rows_per_cache: %d\n", n_size_pad_byte, 
    ...
    for (unsigned int row_index = 0; row_index < rows_per_tasklet;) {
        ...
        for(unsigned int y_index = 0; (y_index<rows_per_cache) && (row_index<rows_per_tasklet); y_index++, \
            ...
            int16_t sigmoid = sigmoid_LUT(dot_product_t >> SHIFT_AMOUNT); 
            ...
            # if PRINT
            if(row_index < 100) {
                printf("%d, x_index: %d, dot_product dpu: %d, sigmoid_dpu: %d, %d\n",row_index, \
                    x_index, dot_product_t, sigmoid, (-dot_product_t) + (SIGMOID_BOUNDARY<<SHIFT_AMOUNT)); 
                printf("X at DPU: "); 
                for (uint32_t each_attribute = 0; each_attribute < n_size_pad; each_attribute++) {
