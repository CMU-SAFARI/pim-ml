...
static int32_t dot_product(T *bufferX, int16_t *bufferW, uint32_t length) {
    ...
    int32_t temph = 0; 
    for (unsigned int i = 0; i < length; i++) {
        __builtin_mul_ul_ul_rrr(templ, bufferX[i], bufferW[i]); 
        __builtin_mul_sh_ul_rrr(temph, bufferW[i], bufferX[i]); 
        result += (temph << 8) + templ; 
    }
    return result; 
}

static int32_t mul_int8_int16(uint8_t x, int16_t y) {
    int32_t templ, temph; 
    __builtin_mul_ul_ul_rrr(templ, x, y); 
    __builtin_mul_sh_ul_rrr(temph, y, x); 
...
int main() {
    ...
    for (unsigned int row_index = 0; row_index < rows_per_tasklet;) {
        ...
        for(unsigned int y_index = 0; (y_index<rows_per_cache) && (row_index<rows_per_tasklet); y_index++, \
            ...
            // compute dot product
            int32_t dot_product_t = dot_product(cache_X + x_index, cache_W, n_size) >> SHIFT_AMOUNT; 
