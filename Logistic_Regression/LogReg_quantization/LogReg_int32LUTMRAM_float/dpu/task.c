/*
 * LogReg_2.1
 * Compute gradient of MSE loss function with multiple tasklet ver1.2
 * Look-up-table sogmoid, accessed by mram_read
 */
#include <stdint.h>
#include <stdio.h>
#include <defs.h>
...
// if the boundary is -+20, size of LUT = 2*20*2048*4 = 320KB for int32 
__mram_noinit T SIGMOID_LUT[SIGMOID_BOUNDARY<<(SHIFT_AMOUNT+1)]; 
...
static T sigmoid_LUT(T x){
    ...
    // Query LUT 
    uint32_t lut_index = (x < 0) ? ((-x) + (SIGMOID_BOUNDARY<<SHIFT_AMOUNT)) : x; 
    __dma_aligned uint64_t temp; 
    mram_read((__mram_ptr void const*) (SIGMOID_LUT+lut_index-1), &temp, 8); 
...
int main() {
    ...
    for (unsigned int row_index = 0; row_index < rows_per_tasklet;) {
        ...
        for(unsigned int y_index = 0; (y_index<rows_per_cache) && (row_index<rows_per_tasklet); y_index++, \
            ...
            T sigmoid = sigmoid_LUT(dot_product_t >> SHIFT_AMOUNT); 
            # endif
